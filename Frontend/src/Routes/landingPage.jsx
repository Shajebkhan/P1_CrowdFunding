import React, { useEffect, useState } from "react";
import { toast } from "react-toastify";
import NavBar from "../Components/navbar_Landing";
import ImageLanding from "../Components/image_landing";
import OnGoingCampaigns from "../Components/onGoingCampaigns";
import WhyUs from "../Components/whyUsSection";
import NobelCauseComponent from "../Components/nobelCauseComponent";
import ProudToDonate from "../Components/proudToDonateComponent";
import ScrollToTop from "../Components/scrollToTop";
import { getAllCampaigns } from "../services/campaign";
import { compare } from "../utills/math";
import education from "../Components/assets/education.jpg";
import medical from "../Components/assets/medical.jpg";
import shelter from "../Components/assets/shelter.jpg";

const LandingPage = (props) => {



  const [data, setData] = useState([{
    id: "abc1",
    title: "Hope for Every Child – Education Fund",
    description:
      "Millions of children lack access to quality education due to financial hardships. This campaign aims to provide scholarships, school supplies, and learning resources to underprivileged students, giving them a chance at a brighter future. Your donation can help a child stay in school and build a better tomorrow.",
    image: education, alt: "education",
    requiredAmount: "1000",
  },
  {
    id: "abc2",
    title: "Emergency Medical Aid – Save Lives Now",
    description:
      "Medical emergencies can strike anytime, and many families struggle to afford life-saving treatments. This campaign helps cover medical expenses, surgeries, and urgent healthcare needs for those in crisis. Every contribution brings hope and healing.",
    image: medical, alt: "medical",
    requiredAmount: "10000",
  },
  {
    id: "abc3",
    title: "Shelter for the Homeless – A Roof for Everyone",
    description:
      "Homelessness is a growing crisis, and this campaign is dedicated to providing safe housing, food, and rehabilitation programs for individuals and families in need. Join us in giving someone the comfort and security of a home.",
    image: shelter, alt: "shelter",
    requiredAmount: "20000",
  },
  ]);

  const [loading, setLoading] = useState(true);




  useEffect(() => {
    async function getData() {
      const { data, err } = await getAllCampaigns();
      if (err === undefined) {
        setLoading(false);
        data.sort(compare);
        if (data.length > 4) {
          setData(data.slice(0, 4));
        } else {
          setData(data);
        }
      } else {
        if (err.response && err.response.data) {
          toast.error(err.response.data.message);
        } else toast.error("Something went wrong");
      }
    }
    getData();
    return null;
  }, []);
  const handleClick = (p) => {
    let url = "/campaign/" + p;
    props.history.push(url);
  };

  return (
    <React.Fragment>
      <NavBar />
      <ScrollToTop />
      <ImageLanding />
      <WhyUs />
      <OnGoingCampaigns
        handleClick={handleClick}
        data={data}
        loading={loading}
      />
      <NobelCauseComponent />
      <ProudToDonate />
    </React.Fragment>
  );
};

export default LandingPage;


/*const data = [
  {
    id: "abc1",
    title: "This is title of crownd funding",
    description:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Itaque dicta, accusamus sapiente ad aspernatur odio. Aspernatur, eum. Dolorem consequuntur molestias voluptatem reiciendis, deserunt unde placeat soluta perferendis exercitationem sit? Porro?",
    image:
      "data:image/jpeg;base64,",
    requiredAmount: "1000",
  },
  {
    id: "abc2",
    title: "This is title",
    description:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Itaque dicta, accusamus sapiente ad aspernatur odio. Aspernatur, eum. Dolorem consequuntur molestias voluptatem reiciendis, deserunt unde placeat soluta perferendis exercitationem sit? Porro?",
    image:
      "data:image/jpeg;base64,",
    requiredAmount: "1000",
  },
  {
    id: "abc3",
    title: "This is title",
    description:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Itaque dicta, accusamus sapiente ad aspernatur odio. Aspernatur, eum. Dolorem consequuntur molestias voluptatem reiciendis, deserunt unde placeat soluta perferendis exercitationem sit? Porro?",
    image:
      "data:image/jpeg;base64,",
    requiredAmount: "1000",
  },
  {
    id: "abc4",
    title: "This is title",
    description:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Itaque dicta, accusamus sapiente ad aspernatur odio. Aspernatur, eum. Dolorem consequuntur molestias voluptatem reiciendis, deserunt unde placeat soluta perferendis exercitationem sit? Porro?",
    image:
      "data:image/jpeg;base64,",
    requiredAmount: "1000",
  },
];*/